...
namespace seedengine {
    ...
    OpenGLWindow::OpenGLWindow(const WindowProperties& properties) : Window(properties, new OpenGLRenderer()) {
        ...
        m_glWindow = glfwCreateWindow(width, height, m_properties.title.c_str(),
            (m_properties.fullscreen && m_properties.borderless) ? glMonitor : nullptr, nullptr);
        ...
        if (m_properties.fullscreen && !m_properties.borderless) glfwMaximizeWindow(m_glWindow);
        if (!m_properties.fullscreen && m_properties.borderless) {
            int x = (glVidMode->width - (int)width) / 2;
            int y = (glVidMode->height - (int)height) / 2;
            glfwSetWindowMonitor(m_glWindow, nullptr, x, y, width, height, glVidMode->refreshRate);
        }
        ...
        if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) {
            // TODO: Error output to graphics logger here (GLAD failed)
            throw -2;
        }
